...
//        if(p<0) p+=(6*h);
    ...
    int heuristic(){
        ...
        vector<int>  my_mobility, opponent_mobility;
        int my_prod = 1, op_prod =1;
        for(int i =0;i< 5;i++)
        {
            if(ring_self.at(i).first != -1)
            {
                my_mobility.push_back(possible_paths(ring_self.at(i).first,ring_self.at(i).second).size());
                my_prod *= my_mobility.at(i);
            }
            if(ring_opponent.at(i).first != -1)
            {
                opponent_mobility.push_back(possible_paths(ring_opponent.at(i).first,ring_opponent.at(i).second).size());
                op_prod *= opponent_mobility.at(i);
            }
        }

        out = out + my_prod - op_prod;

...
vector<Game> remove_rows(vector<pair<pair<int, int>, pair<int, int>>> removal, const Game &game){
    ...
    for(pair<pair<int, int>, pair<int, int>> option: removal){
        ...
        for(const Game &x: current_possibilities) {
            do_remove(x, option, future_possibilities);
        }
        current_possibilities.resize(0);
        for(const Game &x:future_possibilities){

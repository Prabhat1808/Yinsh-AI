...
def td_leaf(f, r, w, lemda, alpha):
    ...
    for i in range(m):
        diff = 0
        for t in range(N-1):
            diff += ((f[t+1][i] - f[t][i]) * sum([(lemda ** (j-t)) * d[t] for j in range(t, N-1)]))
...
if __name__ == "__main__":
    ...
    for p in range(200):
        ...
        os.system("./try.sh")
        os.system("cp logs_1.txt "+fl1)
        os.system("cp logs_2.txt "+fl2)
